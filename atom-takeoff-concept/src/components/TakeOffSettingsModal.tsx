...
import { TAKEOFF_SAVED_PRESETS } from '../mock/takeoffData'
import {
  X,
...
const TAKEOFF_HUB_DESKS: HubDesk[] = [
  { id: 'hd-1', name: 'Hub Desk 01 (LHR / Transatlantic)', operator: 'A. Yıldız', assignedCount: 12 },
  { id: 'hd-2', name: 'Hub Desk 02 (Middle East & Asia)', operator: 'R. Çelik', assignedCount: 18 },
  { id: 'hd-3', name: 'Hub Desk 03 (Europe Wide-body)', operator: 'S. Arslan', assignedCount: 14 },
  { id: 'hd-4', name: 'Hub Desk 04 (Domestic & Regional)', operator: 'K. Erdem', assignedCount: 22 },
]

interface TakeOffSettingsModalProps {
  open: boolean
  onClose: () => void
...
export const TakeOffSettingsModal: React.FC<TakeOffSettingsModalProps> = ({
  open,
  onClose,
  columns,
  onToggleColumn,
  ...
  const [savedFilters, setSavedFilters] = useState<SavedFilter[]>(TAKEOFF_SAVED_PRESETS)
  const [newPresetName, setNewPresetName] = useState('')
  const [editingPresetId, setEditingPresetId] = useState<string | null>(null)
  const [renameInput, setRenameInput] = useState('')
  const [desksState, setDesksState] = useState<HubDesk[]>(hubDesks)

  const isDark = theme === 'dark'

  const [calculatedCols] = useState([
  ...
  const handleAssignDeskInternal = (deskId: string) => {
    if (onAssignHubDesk) {
      onAssignHubDesk(deskId)
    }
    setDesksState((prev) =>
      prev.map((d) => (d.id === deskId ? { ...d, assignedCount: d.assignedCount + 1 } : d))
    )
  }
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-base font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>ATOM TakeOff Operations Settings</h2>
              <p className={`text-xs font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                TakeOff UI Grid layout, hub desk roster, formula columns and saved presets
              </p>
            </div>
          </div>
          <button
        ...
        <div
          className={`flex border-b px-6 ${
            isDark ? 'border-[#1E2D48] bg-[#131B2E]' : 'border-slate-200 bg-white'
          }`}
        >
          {[
        ...
        {/* Tab Body */}
        <div className="flex-1 overflow-y-auto p-6 space-y-6 takeoff-scrollbar">
              ...
              >
                <div>
                  ...
                  <div className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                    Pin flagged watchlist flights to the top of the TakeOff matrix grid
                  </div>
                </div>
                <button
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {desksState.map((desk) => (
                    <div
                      key={desk.id}
                      className={`p-3.5 border rounded-xl shadow-2xs flex items-center justify-between ${
                        isDark ? 'bg-[#0B0F19] border-[#1E2D48]' : 'bg-slate-50 border-slate-200'
                      }`}
                    >
                      <div>
                      ...
                      <button
                        onClick={() => handleAssignDeskInternal(desk.id)}
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-xs font-semibold flex items-center justify-between transition-all ${
                      col.visible
                        ...
                        : isDark
                        ? 'bg-[#0B0F19] border-[#1E2D48] text-slate-500 opacity-60'
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className={`flex-1 px-3 py-2 border rounded-xl text-xs font-semibold outline-none focus:border-[#C8102E] ${
                      isDark ? 'bg-[#131B2E] border-[#1E2D48] text-white' : 'bg-white border-slate-300 text-slate-900'
                    }`}
                  />
                  <button
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {savedFilters.map((preset) => (
                    <div
                      key={preset.id}
                      className={`p-3.5 border rounded-xl shadow-2xs flex items-center justify-between gap-3 ${
                        isDark ? 'bg-[#0B0F19] border-[#1E2D48]' : 'bg-slate-50 border-slate-200'
                      }`}
                    >
        ...
        >
          <button
            onClick={onClose}
            className="px-6 py-2 bg-[#C8102E] text-white font-extrabold text-xs rounded-xl shadow-2xs hover:bg-red-700 transition-colors"
          >
            Done & Apply
          </button>
        </div>
