...
export const TakeOffSidebar: React.FC<TakeOffSidebarProps> = ({
  collapsed,
  onToggleCollapse,
  activeNav,
  onSelectNav,
  ...
  return (
    <aside
      className={`relative flex flex-col transition-all duration-200 ease-in-out z-30 select-none ${
        ...
        isDark
          ? 'bg-[#131B2E] border-r border-[#1E2D48] text-[#F8FAFC]'
          : 'bg-white border-r border-slate-200 text-slate-900 shadow-2xs'
      }`}
    >
      {/* TakeOff UI Branding Header */}
      <div
      ...
      >
        <div className="flex items-center gap-3 overflow-hidden">
          {/* Main Brand Logo: THYlogo.png */}
          <div className="w-20 h-20 rounded-xl flex items-center justify-center flex-shrink-0 bg-[#C8102E]/10 p-1.5 border border-[#C8102E]/25 shadow-2xs">
            <img
              src="/THYlogo.png"
      ...
      {/* Nav Section Links */}
      <div className="flex-1 overflow-y-auto py-3 takeoff-scrollbar">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${
                        isActive
                          ? isDark
                            ? 'bg-[#C8102E]/20 text-white border border-[#C8102E]/50 shadow-2xs'
                            : 'bg-red-50 text-[#C8102E] border border-red-200 shadow-2xs'
                          : isDark
                          ? 'text-slate-300 hover:bg-[#1E253B] hover:text-white'
        ...
        <div className={`px-2 pt-2 border-t ${isDark ? 'border-[#1E2D48]' : 'border-slate-200'}`}>
          {!collapsed && (
            <div
              ...
              <div className="flex items-center justify-between flex-1">
                ...
                <span
                  className={`text-[9px] font-mono font-bold px-1.5 py-0.2 rounded ${
                    isDark ? 'bg-[#1E253B] text-amber-400' : 'bg-slate-200 text-indigo-700'
                  }`}
                >
                  {isDark ? 'DARK' : 'LIGHT'}
                </span>
          ...
          <button
            ...
            title={collapsed ? 'TakeOff Settings' : undefined}
          >
            <Settings className={`w-4 h-4 flex-shrink-0 ${isDark ? 'text-slate-400' : 'text-slate-500'}`} />
            {!collapsed && <span>TakeOff Settings</span>}
          </button>
        </div>
      </div>
        ...
        {!collapsed && (
          <div className="flex-1 overflow-hidden leading-tight">
            ...
            <div className={`text-[10px] font-medium truncate flex items-center gap-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              <Radio className="w-2.5 h-2.5 text-emerald-400 takeoff-pulse" /> OCC Controller
            </div>
          </div>
        )}
        {!collapsed && (
