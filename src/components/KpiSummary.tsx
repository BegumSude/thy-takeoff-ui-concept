...
export const KpiSummary: React.FC<KpiSummaryProps> = ({ flights, theme = 'dark' }) => {
  ...
  const cards = [
    {
      ...
      bgIcon: isDark ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-700',
    },
    {
    ...
    },
    {
      label: 'ACTIVE DELAYS',
      value: delayed,
      subText: `${Math.round((delayed / (total || 1)) * 100)}% of active hub flights`,
      icon: Clock,
