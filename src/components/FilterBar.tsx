...
import { FilterState, FlightStatus, OperationalFlag, OperationalIcon } from '../types/atom'
...
export const FilterBar: React.FC<FilterBarProps> = ({
  ...
  // Collapsed by default for maximum table space
  ...
  const selectClass = `w-full rounded-xl px-2 py-1 text-xs font-semibold outline-none focus:border-[#C8102E] transition-colors ${
    isDark
      ? 'bg-[#0F172A] border border-[#334155] text-white'
  ...
  return (
    <div
      className={`rounded-xl shadow-2xs mb-4 transition-all duration-200 border ${
      ...
      <div className="p-3 flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto custom-scrollbar py-0.5">
          {ALL_STATUSES.map((status) => {
            ...
            const count = status === 'All' ? totalCount : activeStatusCount[status] || 0
            const cfg = status !== 'All' ? STATUS_CONFIG[status as FlightStatus] : null

            return (
              <button
                key={status}
                onClick={() => onFilterChange({ status })}
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold whitespace-nowrap transition-all border ${
                  isActive
                    ? status === 'All'
                      ? isDark
                        ? 'bg-white text-slate-950 border-white shadow-2xs font-extrabold'
                        : 'bg-slate-900 text-white border-slate-900 shadow-2xs font-extrabold'
                      : 'border-current shadow-2xs font-extrabold'
                    : isDark
                    ? 'bg-[#0F172A] text-slate-300 border-[#334155] hover:bg-slate-700/50 hover:text-white'
                    : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:text-slate-900'
                }`}
                style={
                  isActive && cfg
                    ? { color: cfg.fg, backgroundColor: cfg.bg, borderColor: cfg.border }
                    : undefined
                }
              >
                {cfg && (
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-extrabold transition-all border ${
              expanded || activeChips.length > 0
                ...
                : isDark
                ? 'bg-[#0F172A] text-slate-300 border-[#334155] hover:bg-slate-700/50'
          ...
          {activeChips.length > 0 && (
            <button
              ...
              className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold border transition-colors ${
                isDark
                  ? 'text-slate-300 bg-[#0F172A] hover:bg-slate-700/60 border-[#334155]'
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border transition-colors ${
              isDark
                ? 'text-slate-200 bg-[#0F172A] border-[#334155] hover:bg-slate-700/60'
      ...
      {activeChips.length > 0 && !expanded && (
        <div
          className={`px-3 pb-2.5 flex items-center gap-1.5 flex-wrap border-t pt-2 fade-in ${
            isDark ? 'border-[#334155]/60' : 'border-slate-100'
          }`}
        >
